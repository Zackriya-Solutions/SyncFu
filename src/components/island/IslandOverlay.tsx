...
export function IslandOverlay() {
  ...
  // Island action path == card action path VERBATIM (T5a parity, no new
  // transport): a button click drives the SAME `action_callback` command ->
  ...
  const handleAction = useCallback((notificationId: string, actionId: string) => {
    invoke("action_callback", { notificationId, actionId }).catch((err) =>
